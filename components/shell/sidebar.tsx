...
interface SidebarProps {
  onNavigate?: () => void;
  /** Hide the top brand row (drawer already shows a header) */
  hideBrand?: boolean;
}

export function Sidebar({ onNavigate, hideBrand = false }: SidebarProps) {
  ...
  return (
    <nav
      aria-label="Primary"
      className="flex h-full min-h-0 flex-col gap-4 px-3 py-4"
    >
      {!hideBrand && (
        <Link
          href="/dashboard"
              ...
              {items.map(({ href, label, icon: Icon, shortcut }) => {
                ...
                return (
                  ...
                  >
                    ...
                    {shortcut && (
                      <kbd className="hidden rounded border border-border bg-bg px-1 font-mono text-[10px] text-ink-3 lg:inline">
                        {shortcut}
                      </kbd>
                    )}
                  </Link>
      ...
      <div className="flex shrink-0 items-center justify-between border-t border-border pt-3 px-1">
